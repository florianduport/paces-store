    ...
    DatabaseHelper.getDatabase(function(db) {
      db.collection("Configurations", function(err, configurations) {
        ...
        if (err || !configurations) {
          LoggerHelper.logError("marketplace", "Configuration collection couldn't be found", options, err);
          return options.done(LocalConfig);
        }
        configurations.findOne({
          application: options.application
        }, function(err, applicationConfig) {
          if (err || !applicationConfig) {
            LoggerHelper.logError("marketplace", "Application configuration document couldn't be found", options, err);
            return options.done(LocalConfig);
          }
