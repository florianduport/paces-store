...
var AccountModel = {
  ...
  signIn: function(username, password, req, done) {
    ...
    ServiceHelper.getService("customer", "authenticateCustomer", {
      data: {
        username: username,
            ...
            if (response) {
              ...
              ServiceHelper.getService("payment", "createWallet", {
                ...
                if (paymentInfos === undefined) {
                  ...
                  ServiceHelper.getService("customer", "createCustomer", {
                    data: {
                      username: form.username,
  ...
  forgottenPassword: function(req, callback) {
    ...
    if (req.body.username !== undefined) {
      ...
      ServiceHelper.getService("customer", "getFullCustomerByUsername", {
        data: {
          username: req.body.username
        },
        method: "POST"
        ...
        if (user) {
          ...
          ServiceHelper.getService("customer", "createForgottenPasswordToken", {
            data: {
              username: req.body.username,
              token: token
            },
  ...
  displayChangePassword: function(req, callback) {
    ...
    if (req.params.userId !== undefined && req.params.token !== undefined) {
      ...
      ServiceHelper.getService("customer", "getCustomerById", {
        data: {
  ...
  changePassword: function(req, callback) {
    ...
    if (req.body.newPassword !== undefined) {
      ...
      //check if token is valid
      ServiceHelper.getService("customer", "changePassword", {
        data: {
          username: req.body.username,
